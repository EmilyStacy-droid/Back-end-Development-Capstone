...
def songs(request):
    songs_url = "http://songs-sn-labs-emily8745.labs-prod-openshift-san-a45631dc5778dc6371c67d206ba9ae5c-0000.us-east.containers.appdomain.cloud/song" 
    songs = req.get(songs_url).json()
    return render(request, "songs.html", {"songs": songs["songs"]})


def photos(request):
    photos_url = "https://pictures.1aqemg0ipve0.us-south.codeengine.appdomain.cloud/picture"
    photos = req.get(photos_url).json()
...
def logout_view(request):
    # use django.contrib.auth module 
    logout(request)
